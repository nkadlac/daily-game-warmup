class GamePicker {
    ...
    async init() {
        ...
        setTimeout(() => {
            ...
            if (activeMarker && sliderHandle) {
                console.log('Setting initial slider position to:', activeMarker.dataset.value);
                // Temporarily disable transitions for initial positioning
                const originalTransition = sliderHandle.style.transition;
                sliderHandle.style.transition = 'none';
                this.updateSliderPosition(activeMarker.dataset.value);
                // Restore transitions after positioning
                setTimeout(() => {
                    sliderHandle.style.transition = originalTransition || '';
                }, 50);
            }
        }, 100);
    }
    ...
    applyPreferences(preferences) {
        ...
        if (preferences.timeCommitment) {
            ...
            // Disable transition for initial preference application
            const sliderHandle = document.querySelector('.slider-handle');
            if (sliderHandle) {
                const originalTransition = sliderHandle.style.transition;
                sliderHandle.style.transition = 'none';
    ...
    toggleSavedGame(gameId) {
        ...
        // Update only the specific button that was clicked, don't refresh entire view
        this.updateSaveButtonsForGame(gameId, !wasSaved);
    }

    updateSaveButtonsForGame(gameId, isSaved) {
        // Update star buttons in results view
        const starButtons = document.querySelectorAll(`button[onclick*="toggleSavedGame('${gameId}')"]`);
        starButtons.forEach(button => {
            if (button.classList.contains('star-btn')) {
                // Update star icon
                button.innerHTML = isSaved ? '⭐️' : '☆';
            } else if (button.classList.contains('save-btn')) {
                // Update save button in leaderboard
                button.textContent = isSaved ? '❤️ SAVED' : 'SAVE GAME';
                if (isSaved) {
                ...
                // Update save button in old card format
    ...
    shareList() {
        ...
        if (isMobile && navigator.share) {
            // Mobile: use native share API to open app picker
            navigator.share({
            ...
            this.copyToClipboardWithFeedback(shareText, 'share-list');
        }
    }
    ...
    copyToClipboardWithFeedback(text, buttonId) {
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(text).then(() => {
                ...
                this.fallbackCopyToClipboardWithFeedback(text, buttonId);
            });
        } else {
            this.fallbackCopyToClipboardWithFeedback(text, buttonId);
        }
    ...
    fallbackCopyToClipboardWithFeedback(text, buttonId) {
        const button = document.getElementById(buttonId);
        const originalText = button ? button.textContent : '';
